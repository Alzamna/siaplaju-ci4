...
class Googlemaps {
	...
	var $apiKey						= 'AIzaSyCWXmcIsicmh2yZMIPf6_rtL8nk63X8Vcg'; 						// If you've got an API key you can use it by passing this parameter. Setup an API key here: https://code.google.com/apis/console
	...
	var $geocodeCaching				= '';						// If set to TRUE will cache any geocode requests made when an address is used instead of a lat/long. Requires DB table to be created (see documentation)
	...
	function add_polyline($params = array())
	{
		...
		if (count($polyline['points'])) {
			...
			$polyline_output .= '
				...
				var path = polyline_'.count($this->polylines).'.getPath();    
				for (var i = 0; i < polyline_plan_'.count($this->polylines).'.length; i++) {
					var loc = polyline_plan_'.count($this->polylines).'[i];
			...
			*/
			...
			if ($polyline['content']!="") {
				...
				$polyline['content'] = str_replace('\"', '"', $polyline['content']);
				$polyline['content'] = str_replace('"', '\"', $polyline['content']);
			
				$polyline_output .= '
				createInfoWindow(polyline_'.count($this->polylines).', "'.$polyline['content'].'");
				function createInfoWindow(polyline_'.count($this->polylines).', content) {
					google.maps.event.addListener(polyline_'.count($this->polylines).', "click", function(event) {
	...
	function create_map()
	{
		...
		$this->output_js_contents .= '
			...
			var polygonArray = [];
			function CenterControl(controlDiv, '.$this->map_name.') {
				...
				controlUI.title = "Klik untuk menuju lokasi saya";
				...
				controlText.innerHTML = "Lokasi Saya";
				...
				controlUI.addEventListener("click", function() {
					if(navigator.geolocation) {
						navigator.geolocation.getCurrentPosition(function(position) {
							...
							'.$this->map_name.'.setCenter(pos);
							'.$this->map_name.'.setZoom(17);
			...
			if (count($this->kmlLayerURL))
			{
				...
				foreach ($this->kmlLayerURL as $kmlLayerURL)
				{
					...
					$this->output_js_contents .= '
						...
						kmlLayer_'.$i.'.addListener("click", function(kmlEvent) {
		...
		function createMarker_'.$this->map_name.'(markerOptions) {
			//var infowindow = new google.maps.InfoWindow(markerOptions).open(map);
		...
		$this->output_js_contents .= '
		function ControlLokasi(){
			var centerControlDiv = document.createElement("div");
			var centerControl = new CenterControl(centerControlDiv, '.$this->map_name.');

			centerControlDiv.index = 1;
			centerControlDiv.style["padding-top"] = "10px";
			'.$this->map_name.'.controls[google.maps.ControlPosition.TOP_CENTER].push(centerControlDiv);
		}
		';
		
		$this->output_js_contents .= '
		function FormSearchLokasi(){
			var autocompleteInput = document.getElementById("cari_lokasi");
			map.controls[google.maps.ControlPosition.TOP_CENTER].push(autocompleteInput);
		}
		';
		
		$this->output_js_contents .= '
		function PlacesLokasi(){
		...
		$this->output_js_contents .= '
		function garis_batas(){
			garis_batas = new google.maps.KmlLayer({
				url: "https://sijari.duamedia.net/assets/layer/batas_administrasi.kmz",
				map: map,
				preserveViewport: true,
			});
		}
		...
		if ($this->loadAsynchronously) {
			...
			google.maps.event.addDomListener(window, "resize", function() {
				var center = '.$this->map_name.'.getCenter();
				google.maps.event.trigger('.$this->map_name.', "resize");
				'.$this->map_name.'.setCenter(center); 
			});
			';
		}
